...
python3 ../viz_hyperparams.py --treat-e0-as-variable --dir . --outdir .
...
../rl
if [ -f "training_curves.csv" ]; then
    mv training_curves.csv "training_a${RL_ALPHA}_g${RL_GAMMA}_e${RL_EPSILON}_finalRewardOFF.csv"
    mv final_scores.csv "final_a${RL_ALPHA}_g${RL_GAMMA}_e${RL_EPSILON}_finalRewardOFF.csv"
else
    echo "ERROR: training_curves.csv not found!"
fi
...
../rl
if [ -f "training_curves.csv" ]; then
    mv training_curves.csv "training_a${RL_ALPHA}_g${RL_GAMMA}_e${RL_EPSILON}_finalRewardON.csv"
    mv final_scores.csv "final_a${RL_ALPHA}_g${RL_GAMMA}_e${RL_EPSILON}_finalRewardON.csv"
else
    echo "ERROR: training_curves.csv not found!"
fi

python3 ../viz_finalreward.py --dir . --outdir .
